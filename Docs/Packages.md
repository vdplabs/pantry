...
2. If weights are already complete in the package dir **or** a shared Hugging Face cache snapshot (`HF_HUB_CACHE` / `HF_HOME` / default `~/.cache/huggingface/hub` / sibling `…/huggingface/hub` next to `PANTRY_DATA=…/pantry`), return `status: ready` immediately — **no empty weights dir is created**.
   Explicit `HF_HUB_CACHE` / `HF_HOME` isolate discovery away from the default user cache (same convention as `huggingface_hub`).
3. Otherwise, if `runtime.hf_repo` is set, download **once** into the shared HF hub cache (not a second copy under `packages/<id>/weights/`).
4. Mark ready when the snapshot has weight shards (mlx-lm `config.json` trees **or** mflux multi-folder `transformer/` + `*.safetensors` layouts).
...
| `vdplabs.z-image-turbo.standard.v1` | `image_gen` | `image_gen` | `standard` | 14 GB | `mflux` | Z-Image-Turbo **4-bit** (`filipstrand/Z-Image-Turbo-mflux-4bit`; aliases `image-standard`; min 10 GB) |
